...
class _PokemonDetailScreenState extends ConsumerState<PokemonDetailScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _previewCard(PokemonDetail p, Function(String) tr) async {
    ...
    try {
      ...
      if (image != null && mounted) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => CardPreviewScreen(imageBytes: image, pokemonName: p.name)));
      }
    } catch (e) {
  ...
  Widget _buildCardForCapture(PokemonDetail p, List<MoveDto> moves, int hp, Function(String) tr) {
    ...
    return MediaQuery(
      ...
      child: Container(
        width: 350, height: 500, padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), gradient: gradient, boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 10)], border: Border.all(color: Colors.white.withOpacity(0.9), width: 8)),
        child: Column(
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              TypeBadge(type: type, small: true, showText: false),
              Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 8.0), child: Text(_displayName(p.name).toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.white, shadows: [Shadow(color: Colors.black45, offset: Offset(1,1), blurRadius: 2)]), textAlign: TextAlign.left, overflow: TextOverflow.ellipsis))),
              Text('HP $hp', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 22, color: Colors.white, shadows: [Shadow(color: Colors.black45, offset: Offset(1,1), blurRadius: 2)])),
            ]),
            const SizedBox(height: 8),
            Container(
              height: 200, width: double.infinity,
              decoration: BoxDecoration(color: Colors.white.withOpacity(0.95), borderRadius: BorderRadius.circular(8), border: Border.all(color: borderColor, width: 4), boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 4, offset: Offset(2,2))]),
            ...
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(color: Colors.white.withOpacity(0.95), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.white.withOpacity(0.5), width: 1)),
                child: Column(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: moves.map((m) {
                  return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Padding(padding: const EdgeInsets.only(top: 2.0), child: TypeBadge(type: m.type, small: true, showText: false)),
                    const SizedBox(width: 10),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(m.name, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16, color: Colors.black)),
                      const SizedBox(height: 2),
                      Text(m.description, style: const TextStyle(fontSize: 11, color: Colors.black87, height: 1.1, fontStyle: FontStyle.italic)),
                    ])),
    ...
    return detailAsync.when(
      ...
      error: (err, stack) => Scaffold(
        ...
        body: ErrorView(
          ...
          onRetry: () {
            ref.invalidate(pokemonDetailProvider(
                PokemonDetailParams(id: widget.id, gen: widget.genContext)
            ));
          },
        ),
  ...
  Widget _buildContent(BuildContext context, {
    ...
    if (detail != null) {
      movesLvl = detail.moves.where((m) => m.learnMethod == 'level-up').toList()..sort((a,b) => a.level.compareTo(b.level));
      movesTm = detail.moves.where((m) => m.learnMethod == 'machine').toList()..sort((a,b) => a.name.compareTo(b.name));
            ...
            children: [
              Positioned.fill(
                child: Container(
                  ...
                  child: Stack(
                    children: [
                      Positioned(top: -20, right: -60, child: Icon(Icons.catching_pokemon, size: 280, color: Colors.white.withOpacity(0.15))),
                    ],
                  ),
                ),
              ),
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(_displayName(name), style: const TextStyle(fontSize: 36, fontWeight: FontWeight.bold, color: Colors.white, height: 1.2)),
                                    const SizedBox(height: 8),
                                    Wrap(spacing: 8, children: types.map((t) => TypeBadge(type: t, backgroundColor: Colors.white.withOpacity(0.25))).toList()),
                                  ],
                                ),
                              ),
                              Column(
                                ...
                                children: [
                                  Padding(padding: const EdgeInsets.only(top: 8.0), child: Text('#${id.toString().padLeft(3,'0')}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white70))),
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          Switch(
                                            value: _isShiny, onChanged: (val) => setState(() => _isShiny = val),
                                            activeColor: Colors.yellowAccent, activeTrackColor: Colors.yellowAccent.withOpacity(0.5),
                      ...
                      child: detail == null
                        ...
                        children: [
                          ...
                          TabBar(
                            ...
                            indicatorColor: color, indicatorSize: TabBarIndicatorSize.label,
                            indicatorWeight: 3, labelStyle: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14),
                            isScrollable: true,
                            tabs: [
                              Tab(text: tr('about')), Tab(text: tr('stats')), Tab(text: tr('evolutions')),
                              Tab(text: tr('moves')), Tab(text: tr('locations')), Tab(text: tr('megas')), Tab(text: tr('forms')),
                            ],
                          ),
  ...
  void _onSwipe(DragEndDetails d) {
    ...
    if (nextId != null) {
      ...
      try { nextCached = listState.pokemons.firstWhere((p) => p.id == nextId); } catch (_) {
        final allPokes = ref.read(allPokemonProvider).valueOrNull;
        if (allPokes != null) {
          try { nextCached = allPokes.firstWhere((p) => p.id == nextId); } catch (_) {}
        }
      }

      Navigator.of(context).pushReplacement(ScaleFadePageRoute(
          child: PokemonDetailScreen(
            id: nextId, listIds: _ids, initialIndex: nextIdx,
            genContext: widget.genContext, cachedPokemon: nextCached,
          )
      ));
    }
  }
  ...
  List<Widget> _buildMoveCategory({required String title, required List<MoveDto> moves, required int limit, required Function() onShowMore, bool showLvl = false, required Function(String) tr}) {
